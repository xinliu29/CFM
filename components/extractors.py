...
class ExtractSIFT:
    ...
    def run(self, img_path):
        ...
        kp = np.array([[_kp.pt[0]/scale[1], _kp.pt[1]/scale[0], _kp.response, _kp.size, _kp.angle] for _kp in cv_kp])  # N*5
...
class ExtractSuperpoint(object):
    def __init__(self, config):
        default_config = {
            'descriptor_dim': 256,
            'nms_radius': 4,
            'keypoint_threshold': config['det_th'],
