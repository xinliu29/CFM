    ...
    def format_dump_data(self):
        ...
        for seq in self.config['data_seq']:
            ...
            for cur_pair in pairs:
                ...
                cam1 = h5py.File(os.path.join(seq_dir, cam_name_list[index1]), 'r')
                cam2 = h5py.File(os.path.join(seq_dir, cam_name_list[index2]), 'r')
                K1, K2 = cam1['K'][()], cam2['K'][()]
                ...
                R1, R2 = cam1['R'][()], cam2['R'][()]
                t1, t2 = cam1['T'][()].reshape([3, 1]), cam2['T'][()].reshape([3, 1])
                dR = np.dot(R2, R1.T)
                dt = t2 - np.dot(dR, t1)
                ...
                img_path1 = os.path.join('yfcc100m', seq, 'test', name_list[index1])
                img_path2 = os.path.join('yfcc100m', seq, 'test', name_list[index2])
                dump_seq_dir = os.path.join(self.config['feature_dump_dir'], seq, 'test')
                fea_path1 = os.path.join(dump_seq_dir,
                                         name_list[index1].split('/')[-1] + '_' + self.config['extractor']['name']
                                         + '_' + str(self.config['extractor']['num_kpt']) + '.hdf5')

                fea_path2 = os.path.join(dump_seq_dir,
                                         name_list[index2].split('/')[-1] + '_' + self.config['extractor']['name']
                                         + '_' + str(self.config['extractor']['num_kpt']) + '.hdf5')
