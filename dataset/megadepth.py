...
class Megadepth(Dataset):
    def __init__(self,
        ...
        self.train = train
        self.resize = image_size if resize == -1 else resize
        self.min_inliers = min_inliers
        self.max_inliers = max_inliers
        self.random_inliers = random_inliers
        self.invalid_fns = []
        self.keypoints = {}
        self.scene_nvalid_pairs = np.load(str(self.base_path / f'mega_scene_nmatches_{feature_type}.npy'),
    ...
    def build_dataset(self, seed=-1):
        ...
        for scene in tqdm(self.scenes, total=len(self.scenes)):
            ...
            if len(valid_pairs) <= self.pairs_per_scene:
                ...
                try:
                    selected_ids = np.random.choice(len(valid_pairs), self.pairs_per_scene)
                except:
                    continue
        ...
        print('Loaded {:d} pairs'.format(len(self.dataset)))

    def sample_matches(self, idx):
        if not self.train:
            np.random.seed(0)
        pair = self.dataset[idx]
        scene_fn = pair[0]
        pair_id = pair[1]
        match_path = self.base_path / f'matches_sep_{self.feature_type}' / str(scene_fn) / f'{pair_id}.npy'
        data = np.load(str(match_path), allow_pickle=True).item()
        '''
        data: dict = {  'image_path1', 'depth_path1', 'intrinsics1', 'pose1', 
                        'image_path2', 'depth_path2', 'intrinsics2', 'pose2', 
                        'matches'}
        '''
        ...
        feat_fn1 = self.base_path / f'keypoints_{self.feature_type}' / scene / (
                image_path1.split('/')[-1] + f"_{self.feature_type}.npy")
        feat_fn2 = self.base_path / f'keypoints_{self.feature_type}' / scene / (
                image_path2.split('/')[-1] + f"_{self.feature_type}.npy")
        ...
        if kpts1.shape[0] < self.nfeatures:
        ...
        if kpts2.shape[0] < self.nfeatures:
        ...
        if len(matched_ids1) + len(unmatched_ids1) < self.nfeatures or \
                len(matched_ids2) + len(unmatched_ids2) < self.nfeatures:
            return None
        ...
        return {
            'keypoints0': sel_kpts1, 'keypoints1': sel_kpts2,
            'keypoints0_3d': sel_kpts1_3d, 'keypoints1_3d': sel_kpts2_3d,
            ...
            'descriptors0': sel_descs1, 'descriptors1': sel_descs2,
            'scores0': sel_scores1, 'scores1': sel_scores2,
            'intrinsics0': intrinsics1, 'intrinsics1': intrinsics2,
            ...
            'file_name0': image_path1, 'file_name1': image_path2,
        }

    def __getitem__(self, idx):
        for i in range(len(self.dataset)):
            ni = (i + idx) % len(self.dataset)
            output = self.sample_matches(idx=ni)
